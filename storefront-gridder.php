...
final class Storefront_Gridder {
	...
	public function install() {
		...
		$notices[]		= sprintf( __( '%sThanks for installing the Storefront Top Bar extension. To get started, visit the %sCustomizer%s.%s %sOpen the Customizer%s', 'storefront-gridder' ), '<p>', '<a href="' . esc_url( $url ) . '">', '</a>', '</p>', '<p><a href="' . esc_url( $url ) . '" class="button button-primary">', '</a></p>' );
	...
	public function gridder_sf_customize_register( $wp_customize ) {
		...
		$wp_customize->add_section(
			'gridder_sf_custom' , array(
			    'title'      => __( 'Gridder Products Gallery', 'gridderbar' ),
			    'priority'   => 30,
			)
		);
		...
		$wp_customize->add_control(
			new WP_Customize_Control(
				...
				array(
					'label'         => __( 'Number of products to show', 'gridder' ),
					'description' => __( 'Choose the number of products to show', 'gridder' ),
		...
		$wp_customize->add_control(
			...
			'label'       => __( 'Number of columns', 'gridder' ),
			'description' => __( 'Choose the number of columns', 'gridder' ),
			'section'     => 'gridder_sf_custom',
		...
		$wp_customize->add_control(
			...
			'label'       => __( 'Order by', 'gridder' ),
			'description' => __( 'Choose name or date', 'gridder' ),
			'section'     => 'gridder_sf_custom',
			'settings'    => 'gridder_orderby',
			'type'        => 'select',
			'priority'    => 3,
			'choices'     => array(
				'date'       =>  __('Date','gridder'),
				'name'       =>  __('Name','gridder'),
		...
		$wp_customize->add_control(
			new WP_Customize_Control(
				$wp_customize,
				'gridder_order', array(
			'label'       => __( 'Order', 'gridder' ),
			'description' => __( 'Choose Latest or Oldest', 'gridder' ),
			'section'     => 'gridder_sf_custom',
			'settings'    => 'gridder_order',
			'type'        => 'select',
			'priority'    => 4,
			'choices'     => array(
				'ASC'       =>  __('Latest','gridder'),
				'DESC'       =>  __('Oldest','gridder'),
		...
		$wp_customize->add_control(
			...
			'label'       => __( 'Display product price', 'gridder' ),
			'description' => __( 'Choose show or hide to display the product price', 'gridder' ),
			'section'     => 'gridder_sf_custom',
		...
		$wp_customize->add_control(
			...
			'label'       => __( 'Display add to cart button(ajax)', 'gridder' ),
			'description' => __( 'Choose show or hide ajax add to cart button', 'gridder' ),
			'section'     => 'gridder_sf_custom',
		...
		$wp_customize->add_control(
			...
			'label'       => __( 'Display View Product button', 'gridder' ),
			'description' => __( 'Choose show or hide for View Product button ', 'gridder' ),
			'section'     => 'gridder_sf_custom',
		...
		$wp_customize->add_control(
		    new WP_Customize_Control(
		        ...
		        'gridder_mobile_display',
		        array(
		            'label'          		=> __( 'Mobile Display', 'gridderbar' ),
		            'section'        		=> 'gridder_sf_custom',
		            'settings'       		=> 'gridder_mobile_display',
		            'priority'    	  		=> 8,
		            'type'           		=> 'select',
		            'choices'        		=> array(
		                'show-on-mobile'   	=> __( 'Show', 'gridder' ),
		                'hide-on-mobile'  	=> __( 'Hide', 'gridder' )
		            )
		        )
		    )
		);
		...
		if ( is_woocommerce_activated() ) {
			...
			if($is_front_page):
			    ...
			    $args = array(
			    			'post_type'			=> 'product',
			    			'post_status' 		=> 'publish',
			    			'posts_per_page' 	=> $products_number, 			//1n,-1
			    			'orderby' 			=> $orderby, 					//name,date
